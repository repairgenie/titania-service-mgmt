...
// Data functions (insert, update, delete, form) for table ca

// This script and data application were generated by AppGini 22.14
// Download AppGini for free from https://bigprof.com/appgini/download/

function ca_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('ca');
	...
	// hook: ca_before_insert
	if(function_exists('ca_before_insert')) {
		$args = [];
		if(!ca_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('ca', backtick_keys_once($data), $error);
	if($error) {
		$error_message = $error;
		return false;
	}
	...
	update_calc_fields('ca', $recID, calculated_fields()['ca']);

	// hook: ca_after_insert
	if(function_exists('ca_after_insert')) {
		$res = sql("SELECT * FROM `ca` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args = [];
		if(!ca_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('ca', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) ca_copy_children($recID, Request::val('SelectedID'));

	return $recID;
}

function ca_copy_children($destination_id, $source_id) {
	global $Translation;
...
function ca_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('ca', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: ca_before_delete
	if(function_exists('ca_before_delete')) {
		$args = [];
		if(!ca_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `ca` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'invoice_items', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'ca_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'ca_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `ca` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `call_logs` WHERE `call_invoice`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `ca` WHERE `id`='{$selected_id}'", $eo);

	// hook: ca_after_delete
	if(function_exists('ca_after_delete')) {
		$args = [];
		ca_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='ca' AND `pkValue`='{$selected_id}'", $eo);
}

function ca_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('ca', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('ca', $selected_id);
	...
	// hook: ca_before_update
	if(function_exists('ca_before_update')) {
		$args = ['old_data' => $old_data];
		if(!ca_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="ca_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('ca', $data['selectedID'], calculated_fields()['ca']);

	// hook: ca_after_update
	if(function_exists('ca_after_update')) {
		$res = sql("SELECT * FROM `ca` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!ca_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='ca' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function ca_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(is_file(__DIR__ . '/hooks/ca.status.csv')) {
		$status_data = addslashes(implode('', @file(__DIR__ . '/hooks/ca.status.csv')));
	...
	$combo_date_due->MinYear = defined('ca.date_due.MinYear') ? constant('ca.date_due.MinYear') : 1900;
	$combo_date_due->MaxYear = defined('ca.date_due.MaxYear') ? constant('ca.date_due.MaxYear') : 2100;
	...
	if(is_file(__DIR__ . '/hooks/ca.invoice_template.csv')) {
		$invoice_template_data = addslashes(implode('', @file(__DIR__ . '/hooks/ca.invoice_template.csv')));
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='ca' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='ca' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `ca` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'ca_view.php', false);
		}
			...
			$j("#client-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_client__RAND__.value, t: 'ca', f: 'client' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'ca', f: 'client' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/ca_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/ca_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return ca_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return ca_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return ca_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$templateCode = str_replace(
		...
		($selected_id && !$arrPerm['edit'] && ($noSaveAsCopy || !$arrPerm['insert']) ? 
			'<div class="form-control-static">' . $combo_date_due->GetHTML(true) . '</div>' : 
			$combo_date_due->GetHTML()
		), $templateCode);
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'ca_autofill.php?rnd1=$rnd1&mfk=client&id=' + encodeURIComponent('".addslashes($row['client'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'ca_autofill.php?rnd1=$rnd1&mfk=client&id=' + encodeURIComponent(AppGini.current_client{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
		$templateCode .= "\t\t\tbeforeSend: function() { \$j('#client$rnd1').prop('disabled', true); },\n";
		$templateCode .= "\t\t\tcomplete: function() { " . (($arrPerm['insert'] || (($arrPerm['edit'] == 1 && $ownerMemberID == getLoggedMemberID()) || ($arrPerm['edit'] == 2 && $ownerGroupID == getLoggedGroupID()) || $arrPerm['edit'] == 3)) ? "\$j('#client$rnd1').prop('disabled', false); " : "\$j('#client$rnd1').prop('disabled', true); ")." \$j(window).resize(); }\n";
	}
	$templateCode .= "\t\t});\n";
	$templateCode .= "\t};\n";
	...
	$rdata = $jdata = get_defaults('ca');
	if($selected_id) {
		$jdata = get_joined_record('ca', $selected_id);
		if($jdata === false) $jdata = get_defaults('ca');
		$rdata = $row;
	}
	$templateCode .= loadView('ca-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);

	// hook: ca_dv
	if(function_exists('ca_dv')) {
		$args = [];
		ca_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
