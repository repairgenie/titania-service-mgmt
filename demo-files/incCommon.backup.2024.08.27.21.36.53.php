	...
	*/
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'assets' => "`assets`.`asset_ID` as 'asset_ID', IF(    CHAR_LENGTH(`clients1`.`id`) || CHAR_LENGTH(`clients1`.`name`), CONCAT_WS('',   `clients1`.`id`, ' - ', `clients1`.`name`), '') as 'asset_client', `assets`.`asset_type` as 'asset_type', `assets`.`asset_mfr` as 'asset_mfr', `assets`.`asset_model` as 'asset_model', `assets`.`asset_serial` as 'asset_serial', `assets`.`asset_IMEI` as 'asset_IMEI'",
			'workordernotes' => "`workordernotes`.`wonote_ID` as 'wonote_ID', IF(    CHAR_LENGTH(`workorders1`.`wo_ID`) || CHAR_LENGTH(`workorders1`.`wo_Title`), CONCAT_WS('',   `workorders1`.`wo_ID`, ' - ', `workorders1`.`wo_Title`), '') as 'wonote_wo', `workordernotes`.`wonote_note` as 'wonote_note', `workordernotes`.`wonote_author` as 'wonote_author', `workordernotes`.`wonote_timestamp` as 'wonote_timestamp', `workordernotes`.`wonote_editor` as 'wonote_editor', `workordernotes`.`wonote_editorts` as 'wonote_editorts'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'techs' => "`techs` ",
			'assets' => "`assets` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`assets`.`asset_client` ",
			'workordernotes' => "`workordernotes` LEFT JOIN `workorders` as workorders1 ON `workorders1`.`wo_ID`=`workordernotes`.`wonote_wo` ",
	...
	function NavMenus($options = []) {
		...
		if(is_array($arrTables)) {
			foreach($arrTables as $tn => $tc) {
				...
				$tChkHL = array_search($tn, ['invoice_items','workordernotes','technotes','asset_notes']);
