...
function invoice_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('invoice_before_insert')) {
		$args = [];
		if(!invoice_before_insert($data, getMemberInfo(), $args)) {
	...
	update_calc_fields('invoice', $recID, calculated_fields()['invoice']);
	...
	if(strlen(Request::val('SelectedID'))) invoice_copy_children($recID, Request::val('SelectedID'));
...
function invoice_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('invoice', $selected_id, 'delete')) {
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'invoice_items', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="btn btn-danger" value="' . html_attr($Translation['yes']) . '" onClick="window.location = \'invoice_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1&csrf_token=' . urlencode(csrf_token(false, true)) . '\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="btn btn-success" value="' . html_attr($Translation[ 'no']) . '" onClick="window.location = \'invoice_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
...
function invoice_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('invoice', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('invoice', $selected_id);
	...
	if(function_exists('invoice_before_update')) {
		...
		if(!invoice_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('invoice', $data['selectedID'], calculated_fields()['invoice']);
...
function invoice_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if(is_file(__DIR__ . '/hooks/invoice.status.csv')) {
		$status_data = addslashes(implode('', @file(__DIR__ . '/hooks/invoice.status.csv')));
	...
	$combo_date_due->MinYear = defined('invoice.date_due.MinYear') ? constant('invoice.date_due.MinYear') : 1900;
	$combo_date_due->MaxYear = defined('invoice.date_due.MaxYear') ? constant('invoice.date_due.MaxYear') : 2100;
	...
	if(is_file(__DIR__ . '/hooks/invoice.invoice_template.csv')) {
		$invoice_template_data = addslashes(implode('', @file(__DIR__ . '/hooks/invoice.invoice_template.csv')));
			...
			$j("#client-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_client__RAND__.value, t: 'invoice', f: 'client' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'invoice', f: 'client' }; },
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'invoice_autofill.php?rnd1=$rnd1&mfk=client&id=' + encodeURIComponent('".addslashes($row['client'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'invoice_autofill.php?rnd1=$rnd1&mfk=client&id=' + encodeURIComponent(AppGini.current_client{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= loadView('invoice-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);
