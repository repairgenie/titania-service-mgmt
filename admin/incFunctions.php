	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'items' => ['Items', '', 'resources/table_icons/installer_box.png', 'Sales &amp; Billing'],
			'workorders' => ['Work Orders', '', 'table.gif', 'Work Management'],
			'techs' => ['Techs', '', 'table.gif', 'Work Management'],
			'assets' => ['Assets', '', 'table.gif', 'Work Management'],
			'workordernotes' => ['Work Order Notes', '', 'table.gif', 'Work Management'],
			...
			$schema = [
				...
				'workorders' => [
					...
					'wo_ticket' => [
						...
						'info' => [
							'caption' => 'Ticket/Reference Number',
							'description' => '',
						],
					],
					'wo_asset' => [
				...
				'techs' => [
					...
					'techPosition' => [
						'appgini' => "VARCHAR(64) NULL",
						'info' => [
							'caption' => 'Job Role/Position',
							'description' => '',
						],
					],
				],
	...
	*/
	...
	function get_parent_tables($table) {
		...
		$parents = [
			...
			'assets' => [
				'clients' => ['asset_client'],
			],
			'workordernotes' => [
				'workorders' => ['wonote_wo'],
			],
			'technotes' => [
				'techs' => ['technote_tech'],
			],
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'workorders' => [
				'wo_assignedto' => 'SELECT `techs`.`techID`, IF(CHAR_LENGTH(`techs`.`techID`) || CHAR_LENGTH(`techs`.`techName`), CONCAT_WS(\'\', `techs`.`techID`, \' - \', `techs`.`techName`), \'\') FROM `techs` ORDER BY 2',
				'wo_client' => 'SELECT `clients`.`id`, IF(CHAR_LENGTH(`clients`.`id`) || CHAR_LENGTH(`clients`.`name`), CONCAT_WS(\'\', `clients`.`id`, \' - \', `clients`.`name`), \'\') FROM `clients` ORDER BY 2',
				'wo_asset' => 'SELECT `assets`.`asset_ID`, IF(CHAR_LENGTH(`assets`.`asset_client`) || CHAR_LENGTH(`assets`.`asset_serial`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`clients1`.`id`) || CHAR_LENGTH(`clients1`.`name`), CONCAT_WS(\'\',   `clients1`.`id`, \' - \', `clients1`.`name`), \'\'), \' - \', `assets`.`asset_serial`), \'\') FROM `assets` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`assets`.`asset_client` ORDER BY 2',
			],
			'techs' => [
			],
			'assets' => [
				'asset_client' => 'SELECT `clients`.`id`, IF(CHAR_LENGTH(`clients`.`id`) || CHAR_LENGTH(`clients`.`name`), CONCAT_WS(\'\', `clients`.`id`, \' - \', `clients`.`name`), \'\') FROM `clients` ORDER BY 2',
			],
			'workordernotes' => [
				'wonote_wo' => 'SELECT `workorders`.`wo_ID`, IF(CHAR_LENGTH(`workorders`.`wo_ID`) || CHAR_LENGTH(`workorders`.`wo_Title`), CONCAT_WS(\'\', `workorders`.`wo_ID`, \' - \', `workorders`.`wo_Title`), \'\') FROM `workorders` LEFT JOIN `techs` as techs1 ON `techs1`.`techID`=`workorders`.`wo_assignedto` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`workorders`.`wo_client` LEFT JOIN `assets` as assets1 ON `assets1`.`asset_ID`=`workorders`.`wo_asset` LEFT JOIN `clients` as clients2 ON `clients2`.`id`=`assets1`.`asset_client` ORDER BY 2',
			],
			'technotes' => [
				'technote_tech' => 'SELECT `techs`.`techID`, IF(CHAR_LENGTH(`techs`.`techID`) || CHAR_LENGTH(`techs`.`techName`), CONCAT_WS(\'\', `techs`.`techID`, \' - \', `techs`.`techName`), \'\') FROM `techs` ORDER BY 2',
			],
			'tblwopubstatus' => [
				'wopub_WO' => 'SELECT `workorders`.`wo_ID`, IF(CHAR_LENGTH(`workorders`.`wo_ID`) || CHAR_LENGTH(`workorders`.`wo_Title`), CONCAT_WS(\'\', `workorders`.`wo_ID`, \' - \', `workorders`.`wo_Title`), \'\') FROM `workorders` LEFT JOIN `techs` as techs1 ON `techs1`.`techID`=`workorders`.`wo_assignedto` LEFT JOIN `clients` as clients1 ON `clients1`.`id`=`workorders`.`wo_client` LEFT JOIN `assets` as assets1 ON `assets1`.`asset_ID`=`workorders`.`wo_asset` LEFT JOIN `clients` as clients2 ON `clients2`.`id`=`assets1`.`asset_client` ORDER BY 2',
			],
			'asset_notes' => [
