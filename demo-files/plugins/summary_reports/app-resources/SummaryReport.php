...
class SummaryReport {
	...
	private function _process_request($request, $date_format) {
		...
		if($this->barchart_section || $this->piechart_section) {
			$request['order-by'] = $this->request('order-by', 'value', ['label']);
			$request['sorting-order'] = $this->request('sorting-order', 'desc', ['asc']);
		} else {
			$request['order-by'] = $this->request('order-by', 'label', ['value']);
			$request['sorting-order'] = $this->request('sorting-order', 'asc', ['desc']);
		}
	...
	private function periods() {
		if(!isset($this->date_field)) return [[false, false]];
	...
	private function render_report_title(){
		...
		<h1> <?php echo ($report_title = $this->title . ' ' . $from_to_title); ?> 
			<div class="form-group pull-right hidden-print"> 
				...
				<button onclick="exportTableToCSV('<?php echo str_replace([' ', '/', '\\'], '-', trim($report_title)); ?>.csv')" class="btn btn-primary btn-lg"><i class="glyphicon glyphicon-floppy-disk"></i> <?php echo $this->report_translation['export report to csv file'] ?></button>
			</div>	
				...
				<tbody>
					<?php
						if(!is_null($items)) foreach($items as $item) {
							...
							// if this report is not segmented by a date field
							if(
								isset($filter_flag)
								&& !isset($this->date_field)
								&& isset($filterValue)
							) {
								if($filterValue != NOLABEL)
								...
								$linkTitle = html_attr(str_replace(
									array_keys($titleReplace),
									array_values($titleReplace),
									$this->report_translation['view table where label is']
								));
								$value1 = "<a href=\"{$value1_filter}\" title=\"{$linkTitle}\">{$value1}</a>";
							}

							// if this report is segmented by a date field
							if(isset($filter_flag) && isset($this->date_field)) {				
								...
								$linkTitles = $percentDiffs = [];

								for($i = 0; $i < 3; $i++) {
									${'value' . ($i + 1) . '_filter'} = $label_filter . '&' . 
									...
									$titleReplace = [
										...
										'<DATE_FROM>' => $this->{"p{$i}_start"},
										'<DATE_TO>' => $this->{"p{$i}_end"},
									];
									$linkTitles[] = html_attr(str_replace(
										array_keys($titleReplace),
										array_values($titleReplace),
										$this->report_translation['view table where label is and dates between']
									));

									$currVal = 'value' . ($i + 1);
									$currValFilterLink = "{$currVal}_filter";
									${$currVal} = "<a href=\"${$currValFilterLink}\" title=\"{$linkTitles[$i]}\">${$currVal}</a>";

									/* percentage change between current period and main (1st) one */
									if(!$i)
										$percentDiffs[$i] = '';
									else
										$percentDiffs[$i] = $this->change_percent(
											$item['value1'],
											$item['value' . ($i + 1)]
										);
								}
						...
						<tr>
							...
							<?php if(isset($this->date_field)) for($i = 1; $i < 3; $i++) { ?>
								<?php $i1 = $i + 1; ?>
								<?php if($this->{"show_p{$i}"}) { ?>
									<td class="text-right item-value"><?php echo ${"value{$i1}"}; ?></td>
									<td class="percentage text-right"><?php echo $percentDiffs[$i]; ?></td>
								<?php } ?>
							<?php } ?>
						</tr>
							
					<?php } /* end of foreach item */ ?> 
				</tbody>
			</table>
		</div>
